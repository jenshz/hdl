...
set_property CLOCK_DELAY_GROUP BALANCE_CLOCKS_2 \
 ...
 # MCS is synchronous fpga_ref_clk

 set_input_delay -clock [get_clocks fpga_ref_clk] [get_property PERIOD [get_clocks fpga_ref_clk_p]] \
                 [get_ports -regexp -filter { NAME =~  ".*fpga_mcs_in_p.*" && DIRECTION == "IN" }]
